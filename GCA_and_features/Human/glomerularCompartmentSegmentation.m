...
parfor g=1:length(image_dir)
    ...
    % Get precursor mask from HSV transform
    HSV=rgb2hsv(I);
    mesInt=HSV(:,:,2)./HSV(:,:,3);
    mes=mesInt>0.5;
    ...
    % In case an image which is not a glomerulus did not get removed,
    % sometimes these images can crash the algorithm. If this happens just
    % skip the image. 
    try
