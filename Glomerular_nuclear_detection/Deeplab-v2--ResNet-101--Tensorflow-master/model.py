...
This script has been modified to accept a prior weighting value on the network output probabilites. 
"""
...
class Model(object):
	...
	def train(self):
		...
		for step in range(self.conf.num_steps+1):
			...
			print('step {:d} \t loss = {:.3f}, ({:.3f} sec/step)'.format(step, loss_value, duration) + normal_color)
	...
	def test(self):
		...
		for step in range(self.conf.valid_num_steps):
			...
			if step % 100 == 0:
				print('step {:d}'.format(step) + normal_color)
		print('Pixel Accuracy: {:.3f}'.format(self.accu.eval(session=self.sess)) + normal_color)
		print('Mean IoU: {:.3f}'.format(self.mIoU.eval(session=self.sess)) + normal_color)
	...
	def predict(self):
		...
		for step in range(self.conf.test_num_steps):
			...
			im.save(self.conf.out_dir + '/prediction'+'/'+str(prior1) + filename)
			...
			if self.conf.visual:
				...
				im.save(self.conf.out_dir + '/visual_prediction'+'/'+str(prior1) + filename)

			if step % 100 == 0:
				print('step {:d}'.format(step))

		print('The output files has been saved to {}'.format(self.conf.out_dir) + normal_color)
	...
	def predict_setup(self):
		...
		# Create directory
		if not os.path.exists(self.conf.out_dir+'/prediction'+'/'+str(prior1)):
			#os.makedirs(self.conf.out_dir)
			os.makedirs(self.conf.out_dir + '/prediction'+'/'+str(prior1))
			if self.conf.visual:
				os.makedirs(self.conf.out_dir + '/visual_prediction'+'/'+str(prior1))
