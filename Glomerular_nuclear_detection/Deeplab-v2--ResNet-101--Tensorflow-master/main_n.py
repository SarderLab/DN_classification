...
def configure(test_data_list_, out_dir_, test_step_, test_num_steps_, modeldir_, data_dir_, num_steps_, save_interval_, learning_rate_, pretrain_file_, data_list_, batch_size_, input_height_, input_width_, num_classes_,prior_):
	flags = tf.app.flags
	...
	# validation
	flags.DEFINE_integer('valid_step', test_step_, 'checkpoint number for validation')
	flags.DEFINE_integer('valid_num_steps', test_step_, '= number of validation samples')
	...
	flags.DEFINE_float('prior', float(prior_), 'Amount to weight predictions towards the nuclear class (0.5 is balanced)')
...
if __name__ == '__main__':
	...
	parser.add_argument('--prior', dest='prior', type=float, default=0.5,
		help='Prior probability weight amount')
